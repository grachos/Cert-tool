...
export default function Prisma(){
  ...
  const canEdit=['SUPERADMIN','ADMIN','MILL_ADMIN','MANAGER'].includes(user?.role||'');
  const safeRows = Array.isArray(rows) ? rows : [];
  const load=()=>{if(!selectedUocId||selectedUocId==='all')return;api.get('/rspo/prisma-operations',{params:{uocId:selectedUocId}}).then(r=>setRows(Array.isArray(r.data)?r.data:[])).catch(e=>{setRows([]);setError(e.response?.data?.error||'No fue posible cargar las operaciones.');});};
  ...
  const history=async(row:any)=>{try{const{data}=await api.get(`/rspo/prisma-operations/${row.id}/adjustments`);const safe=Array.isArray(data)?data:[];window.alert(safe.length?safe.map((x:any)=>`${new Date(x.createdAt).toLocaleString()}: ${x.adjustmentType} ${x.previousVolumeMt} → ${x.newVolumeMt} · ${x.reason} · ${x.changedByName}`).join('\n'):'Sin cambios registrados.');}catch(err:any){setError(err.response?.data?.error||'No fue posible consultar el historial.');}};
  ...
  const supports=async(row:any)=>{try{const{data}=await api.get(`/rspo/prisma-operations/${row.id}/attachments`);const safe=Array.isArray(data)?data:[];window.alert(safe.length?safe.map((x:any)=>x.originalFileName||x.evidenceId).join('\n'):'Sin soportes adjuntos.');}catch(err:any){setError(err.response?.data?.error||'No fue posible consultar soportes.');}};
  if(!selectedUocId||selectedUocId==='all')return <div className="empty-state card"><h3>Seleccione una UoC</h3><p>Las transacciones se administran por unidad de certificación.</p></div>;
  return <div className="flex-col gap-5 animate-fade-in">
    <section className="card prisma-hero"><div><span className="eyebrow">CONTROL INTERNO</span><h2 className="text-2xl font-bold">Transacciones</h2><p>Preparación, seguimiento y conciliación de operaciones.</p></div><span className="badge badge-pending">Integración oficial pendiente</span></section>
    <div className="integration-note"><strong>Importante:</strong> este módulo conserva el control interno. La conexión con el sistema externo oficial se habilitará cuando existan credenciales y documentación de API.</div>
    <div className="flex-between"><span>{safeRows.length} operaciones internas</span>{canEdit&&<button className="btn btn-primary" onClick={()=>setShow(v=>!v)}>{show?'Cancelar':'+ Nueva operación'}</button>}</div>
    ...
    {show&&<form className="card form-grid" onSubmit={submit}>
      ...
      <input className="form-input" placeholder="Referencia externa (si existe)" value={form.prismaReference} onChange={e=>setForm({...form,prismaReference:e.target.value})}/>
    ...
    {rows.length===0?<div className="empty-state card"><h3>No hay operaciones internas</h3><p>Cree el primer registro cuando tenga una operación que preparar o conciliar.</p></div>:<div className="card p-0"><div className="table-responsive"><table className="w-full min-w-[1100px]"><thead><tr><th>Tipo</th><th>Referencia interna</th><th>Referencia externa</th><th>Producto</th><th>Modelo</th><th>Volumen</th><th>Fecha física</th><th>Límite</th><th>Estado</th><th>Contraparte</th><th>Acciones</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td>{r.operationType}</td><td>{r.internalReference}</td><td>{r.prismaReference||'—'}</td><td>{r.product}</td><td>{r.supplyModel}</td><td>{Number(r.volumeMt)} TM</td><td>{String(r.physicalDate).slice(0,10)}</td><td>{r.deadline?String(r.deadline).slice(0,10):'—'}</td><td>{r.status}</td><td>{r.counterparty||'—'}</td><td><div className="flex gap-1 flex-wrap"><button className="btn btn-sm btn-secondary" onClick={()=>history(r)}>Historial</button><button className="btn btn-sm btn-secondary" onClick={()=>supports(r)}>Soportes</button>{canEdit&&<><button className="btn btn-sm btn-secondary" onClick={()=>attach(r)}>Adjuntar</button><button className="btn btn-sm btn-secondary" onClick={()=>change(r,'CONFIRMED')}>Confirmar</button><button className="btn btn-sm btn-secondary" onClick={()=>change(r,'REMOVED')}>Retirar</button><button className="btn btn-sm btn-secondary" onClick={()=>change(r,'ADJUSTED')}>Ajustar</button></>}</div></td></tr>)}</tbody></table></div></div>}
  </div>;
}
