import { mockComplianceStatuses, mockActivities, mockDashboardStats } from '../data/mock-data';
import { standards } from '../data/standards';

interface DashboardProps {
  onNavigate: (module: 'dashboard' | 'documents' | 'risks' | 'compliance' | 'evidence' | 'automation') => void;
}
...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  return (
    <div className="flex-col gap-6 animate-fade-in">
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="card">
          <div className="flex justify-between items-start mb-4">
            <span className="text-sm text-secondary font-medium uppercase tracking-wide">Cumplimiento Global</span>
          ...
          <div className="flex items-end justify-between">
            <span className="text-3xl font-bold text-primary">{mockDashboardStats.overallCompliance}%</span>
        ...
        <div className="card">
          <div className="flex justify-between items-start mb-4">
            <span className="text-sm text-secondary font-medium uppercase tracking-wide">Docs. por Revisar</span>
          ...
          <div className="flex items-end justify-between">
            <span className="text-3xl font-bold text-primary">{mockDashboardStats.pendingReviews}</span>
        ...
        <div className="card">
          <div className="flex justify-between items-start mb-4">
            <span className="text-sm text-secondary font-medium uppercase tracking-wide">Riesgos Activos</span>
          ...
          <div className="flex items-end justify-between">
            <span className="text-3xl font-bold text-primary">{mockDashboardStats.activeRisks}</span>
            <span className="text-sm text-accent-red font-medium flex items-center gap-1">
              {mockDashboardStats.criticalRisks} Críticos
            </span>
          </div>
        </div>
        ...
        <div className="card">
          <div className="flex justify-between items-start mb-4">
            <span className="text-sm text-secondary font-medium uppercase tracking-wide">Planes Vencidos</span>
          ...
          <div className="flex items-end justify-between">
            <span className="text-3xl font-bold text-accent-red">{mockDashboardStats.overdueActions}</span>
            <span className="text-sm text-secondary font-medium flex items-center gap-1">
              Requieren atención
            </span>
          </div>
      ...
      <div>
        <h3 className="text-lg font-bold text-primary mb-4">Cumplimiento por Norma</h3>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="card col-span-2 p-0 overflow-hidden">
          <div className="p-4 border-b border-gray-200">
            <h3 className="text-base font-bold text-primary">Actividad Reciente</h3>
          </div>
          <div className="flex-col">
            {mockActivities.map((activity, idx) => (
              <div key={activity.id} className={`flex gap-4 p-4 hover:bg-surface-1 transition-colors ${idx !== mockActivities.length - 1 ? 'border-b border-gray-100' : ''}`}>
        ...
        {/* Quick Actions */}
        <div className="card">
          <h3 className="text-base font-bold text-primary mb-4">Acciones Rápidas</h3>
          ...
          <div className="mt-6 p-4 rounded-lg border" style={{ background: 'var(--accent-blue-light)', borderColor: '#bfdbfe' }}>
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="text-sm font-bold" style={{ color: 'var(--accent-blue)' }}>Revisión Automatizada</h4>
                <p className="text-xs text-secondary mt-1">3 documentos pendientes de análisis. ¿Iniciar proceso?</p>
