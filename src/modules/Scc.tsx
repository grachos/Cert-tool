...
export default function Scc() {
  ...
  const tabs: { id: SccTab; label: string }[] = [
    { id: 'dashboard', label: 'Dashboard' }, { id: 'reception', label: 'Recepción' }, { id: 'production', label: 'Producción' },
    { id: 'sales', label: 'Ventas' }, { id: 'stock', label: 'Balance' }, { id: 'prisma', label: 'Prisma' },
    { id: 'control', label: 'Control' }, { id: 'checklist', label: 'Checklist' }, { id: 'uocs', label: 'UoCs' },
  ];
  ...
  const getTypeBadge = (type: string) => {
    ...
    return <span className="badge" style={{ background: s.c, color: s.l }}>{type === 'RECEPTION' ? 'Recepción' : type === 'PRODUCTION' ? 'Prod.' : type === 'SALE' ? 'Venta' : 'Transf.'}</span>;
  };

  const getStatusBadge = (status: string) => {
    if (status === 'COMPLIANT' || status === 'OK') return <span className="badge" style={{ background: 'var(--accent-green-bg)', color: 'var(--accent-green)' }}>Cumple</span>;
    if (status === 'PARTIAL' || status === 'WARN') return <span className="badge" style={{ background: 'var(--accent-gold-bg)', color: 'var(--accent-gold)' }}>Parcial</span>;
    return <span className="badge" style={{ background: '#f1f5f9', color: 'var(--text-muted)' }}>Pendiente</span>;
  };

  if (loading) return <div className="text-center text-muted" style={{ padding: '3rem' }}>Cargando datos SCC...</div>;
  ...
  return (
    <div className="flex-col gap-6 animate-fade-in">
      ...
      {activeTab === 'dashboard' && (
        <div className="flex-col gap-6">
          <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
            <div className="card"><div className="text-sm text-secondary font-medium uppercase tracking-wide">UoCs</div><div className="flex justify-between items-end mt-3"><span className="text-3xl font-bold text-primary">{dashboard?.uocCount || 0}</span><span className="text-sm text-accent-green font-medium">Activas</span></div></div>
            <div className="card"><div className="text-sm text-secondary font-medium uppercase tracking-wide">Transacciones</div><div className="flex justify-between items-end mt-3"><span className="text-3xl font-bold text-primary">{transactions.length}</span><span className="text-sm text-muted font-medium">Total</span></div></div>
            <div className="card"><div className="text-sm text-secondary font-medium uppercase tracking-wide">TM Recibidas</div><div className="flex justify-between items-end mt-3"><span className="text-3xl font-bold text-primary">{sumVol('RECEPTION').toFixed(0)}</span><span className="text-sm text-muted font-medium">RFF + CPO</span></div></div>
            <div className="card"><div className="text-sm text-secondary font-medium uppercase tracking-wide">TM Vendidas</div><div className="flex justify-between items-end mt-3"><span className="text-3xl font-bold text-primary">{sumVol('SALE').toFixed(0)}</span><span className="text-sm text-muted font-medium">CPO + PK</span></div></div>
          </div>
            ...
            {[
              ...
              return (
                <div key={cr.m} className="card" style={{ borderLeft: `4px solid ${modelColors[cr.m]}` }}>
                  ...
                  <div className="flex justify-between text-sm text-secondary mt-2"><span>Disponible: <b>{cr.a.toLocaleString()} TM</b></span><span>Vendido: <b style={{ color: 'var(--accent-red)' }}>{cr.s.toLocaleString()} TM</b></span></div>
                  {cr.p > 0 && <div className="text-sm text-secondary mt-1">Comprado: <b style={{ color: 'var(--accent-green)' }}>{cr.p.toLocaleString()} TM</b></div>}
                  <div className="flex gap-2 mt-4"><button className="btn btn-secondary btn-sm">Registrar Venta</button><button className="btn btn-secondary btn-sm">Registrar Compra</button></div>
                </div>
              );
            })}
          </div>
          <div className="card p-0 overflow-hidden">
            <div className="p-4 border-b bg-surface-1"><h3 className="text-base font-bold text-primary">Certificados RSPO</h3></div>
            <table className="w-full text-left"><thead><tr className="bg-white border-b"><th className="p-4 text-xs font-bold text-secondary uppercase">Modelo</th><th className="p-4 text-xs font-bold text-secondary uppercase">Certificado</th><th className="p-4 text-xs font-bold text-secondary uppercase">Disponible</th><th className="p-4 text-xs font-bold text-secondary uppercase">Vendido</th><th className="p-4 text-xs font-bold text-secondary uppercase">Balance</th><th className="p-4 text-xs font-bold text-secondary uppercase">Estado</th></tr></thead><tbody>
            {[
              { m: 'IP', l: 'Identity Preserved', a: 1250, s: 320, p: 0, c: 'RSPO-IP-2026-001' },
              { m: 'SG', l: 'Segregated', a: 850, s: 500, p: 100, c: 'RSPO-SG-2026-002' },
              { m: 'MB', l: 'Mass Balance', a: 2100, s: 1800, p: 300, c: 'RSPO-MB-2026-003' },
              { m: 'BC', l: 'Book & Claim', a: 500, s: 200, p: 0, c: 'RSPO-BC-2026-004' },
            ].map(cr => <tr key={cr.m} className="border-b hover:bg-surface-1"><td className="p-4"><span className="font-semibold" style={{ color: modelColors[cr.m] }}>{cr.l}</span></td><td className="p-4 text-sm font-mono text-secondary">{cr.c}</td><td className="p-4">{cr.a.toLocaleString()}</td><td className="p-4" style={{ color: 'var(--accent-red)' }}>{cr.s.toLocaleString()}</td><td className="p-4 font-bold">{((cr.a + cr.p - cr.s)).toLocaleString()}</td><td className="p-4"><span className="badge" style={{ background: 'var(--accent-green-bg)', color: 'var(--accent-green)' }}>Activo</span></td></tr>)}
          </tbody></table>
          </div>
        </div>
      )}
      ...
      {activeTab === 'control' && (
        <div className="card p-0 overflow-hidden">
          <div className="p-4 border-b bg-surface-1"><h3 className="text-base font-bold text-primary">Control Interno — Verificación SCC</h3></div>
          <table className="w-full text-left"><thead><tr className="bg-white border-b"><th className="p-4 text-xs font-bold text-secondary uppercase">Item de Control</th><th className="p-4 text-xs font-bold text-secondary uppercase">Frecuencia</th><th className="p-4 text-xs font-bold text-secondary uppercase">Estado</th></tr></thead><tbody>
            {[
              { i: 'Verificar conciliación de entradas RFF vs salidas CPO', f: 'Diario', s: 'OK' },
              { i: 'Validar trazabilidad de lotes en todas las transacciones', f: 'Diario', s: 'OK' },
              { i: 'Confirmar volúmenes vendidos no exceden créditos disponibles', f: 'Semanal', s: 'WARN' },
              { i: 'Revisar GEI declarado vs calculado', f: 'Mensual', s: 'OK' },
              { i: 'Auditar documentos de respaldo (REMs, facturas, certificados)', f: 'Mensual', s: 'PENDING' },
              { i: 'Validar registro de UoCs en PalmTrace', f: 'Trimestral', s: 'OK' },
              { i: 'Verificar todas las transacciones tienen supplyModel', f: 'Diario', s: 'OK' },
      ...
      {activeTab === 'checklist' && (
        <div className="flex-col gap-4">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-bold text-primary">Checklist Pre-Auditoría SCC</h3>
            <span className="text-base font-bold" style={{ color: 'var(--accent-green)' }}>9/12 completados</span>
          </div>
          ...
          <div className="card p-0 overflow-hidden">
            <table className="w-full text-left"><thead><tr className="bg-white border-b"><th className="p-4 text-xs font-bold text-secondary uppercase">Área</th><th className="p-4 text-xs font-bold text-secondary uppercase">Item de Verificación</th><th className="p-4 text-xs font-bold text-secondary uppercase">Obligatorio</th><th className="p-4 text-xs font-bold text-secondary uppercase">Estado</th></tr></thead><tbody>
            {[
              { a: 'Documentación', i: 'Certificados RSPO vigentes para todos los modelos', r: true, d: true },
              { a: 'Documentación', i: 'Procedimiento SCC documentado y aprobado', r: true, d: true },
              { a: 'Documentación', i: 'Registros de capacitación del personal en SCC', r: true, d: true },
              { a: 'Trazabilidad', i: 'Sistema de trazabilidad implementado (lotes, documentos)', r: true, d: true },
              { a: 'Trazabilidad', i: 'Balance de masa por modelo sin déficits', r: true, d: true },
              { a: 'PalmTrace', i: 'Registro activo en PalmTrace / Prisma', r: true, d: true },
              { a: 'PalmTrace', i: 'Transacciones de créditos registradas al día', r: false, d: true },
              { a: 'Ventas', i: 'Facturas con declaración RSPO (modelo de suministro)', r: true, d: true },
              { a: 'Ventas', i: 'Contratos con compradores incluyen cláusula RSPO', r: false, d: true },
              { a: 'Auditoría', i: 'Auditoría interna SCC en últimos 12 meses', r: true, d: false },
              { a: 'Auditoría', i: 'No conformidades anteriores cerradas', r: true, d: false },
              { a: 'Ambiental', i: 'Cálculo de GEI para todas las transacciones', r: false, d: false },
            ].map((it, i) => <tr key={i} className="border-b hover:bg-surface-1" style={{ opacity: it.d ? 1 : 0.5 }}><td className="p-4 text-sm text-secondary">{it.a}</td><td className="p-4 text-sm">{it.i}</td><td className="p-4">{it.r ? <span className="badge" style={{ background: 'var(--accent-red-bg)', color: 'var(--accent-red)' }}>Sí</span> : <span className="badge" style={{ background: 'var(--accent-blue-light)', color: 'var(--accent-blue)' }}>No</span>}</td><td className="p-4">{it.d ? <span className="badge" style={{ background: 'var(--accent-green-bg)', color: 'var(--accent-green)' }}>Listo</span> : <span className="badge" style={{ background: 'var(--accent-gold-bg)', color: 'var(--accent-gold)' }}>Pendiente</span>}</td></tr>)}
          </tbody></table>
          </div>
        </div>
      )}
          ...
          <div className="modal card max-w-xl w-full p-6 animate-scale-in" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreate} className="flex-col gap-4">
              {formMode === 'UOC' ? (<>
                ...
                <div className="flex gap-2"><div className="form-group flex-col gap-1 flex-1"><label className="form-label font-semibold">Modelo</label><select name="supplyModel" className="form-input" required><option value="">Modelo...</option><option value="IP">IP</option><option value="SG">SG</option><option value="MB">MB</option><option value="BC">BC</option></select></div><div className="form-group flex-col gap-1 flex-1"><label className="form-label font-semibold">Volumen (TM)</label><input name="volumeMt" className="form-input" type="number" step="0.001" required /></div></div>
                <div className="form-group flex-col gap-1"><label className="form-label font-semibold">Lote</label><input name="batchRef" className="form-input" /></div>
